...
elbow_plot <- plot(1:length(residvar), residvar, type = 'b', # Elbow plot
...
for (i in 1:10) {
  kmeans_result <- kmeans(pca$x, centers = i, nstart = 50)
  wcss[i] <- kmeans_result$tot.withinss  # Store the WCSS value for each number of clusters
}

# Plot Elbow Plot
plot(1:10, wcss, type = 'b', pch = 19, col = 'lightblue', xlab = 'Number of Clusters', 
...
kmeans_result <- kmeans(pca$x, centers = 5, nstart = 50)# Add cluster assignments to pca.data.summary
...
pca_plot_threeD <- pca_plot_threeD %>%
  # Update the marker color based on the 'Cluster' variable
  add_trace(x = pca_scaled[,1], 
            y = pca_scaled[,2], 
            z = pca_scaled[,3], 
            type = 'scatter3d', mode = 'markers',
            marker = list(color = pca_scaled$Cluster, size = 3),  # Color by cluster
            name = 'Samples')
